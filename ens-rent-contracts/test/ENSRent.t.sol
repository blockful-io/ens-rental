...
contract ENSRentTest is Test {
    ...
    function test_rentDomain() public {
        ...
        uint256 rentPeriod = 1 days;
        (uint256 currentRentPrice,,) = ensRent.getCurrentPrice(tokenId);
        uint256 totalRentPrice = currentRentPrice * rentPeriod;

        ensRent.rentDomain{ value: totalRentPrice }(tokenId, block.timestamp + rentPeriod);

        (,, uint256 maxEnd, address borrower, uint256 rentalEnd,,) = ensRent.rentalTerms(tokenId);
        assertEq(borrower, RENTER);
        assertEq(rentalEnd, block.timestamp + rentPeriod);
        assertLe(rentalEnd, maxEnd);
        vm.stopPrank();
    }
    ...
    function test_rentDomain_ShouldRefundExcessPayment() public {
        ...
        uint256 rentPayment = 100 ether;

        ensRent.rentDomain{ value: rentPayment }(tokenId, block.timestamp + 1 minutes);
